...
| `@app/video-providers` `ProviderSubmissionOutcome` + `VideoGenerationSubmissionProvider` | Implemented (Phase 4C-3B-2C-1, ADR-0035); submission reports certainty as a returned value, not an exception. WaveSpeed and Fake implement it; **no submission-only adapter exists yet** |
