...
**件数: A = 13 件 / B = 11 件 / C = 6 件。**

> **A 分類のうち A-09 のみ、本監査の途中で Orchestrator によって決着済みとなった**（HOLD は解除されない。§1 参照）。したがって列挙は 13 件、うち**人間判断がなお必要なものは 12 件**である。
>
> A-13（CRM / opt-out ストアの所在）は初版監査の後に追加された。T-009 ロードマップへの Tech Lead 指摘 T-06 で検出されたもので、初版の 14 文書からは導けない事項である。
...
| **A-13** | CRM / opt-out ストアの所在 | T-009 ロードマップ §3 領域 C 前提条件（Tech Lead 指摘 T-06 により追加） | アウトリーチ用の個人データをどこに置くかが未定義。外部 SaaS か自社実装かが決まっていない | 外部 SaaS なら越境移転と処理者契約が発生し、プライバシーポリシーと DPA の記載が変わる。自社 DB なら、テナント資産ではない自社の見込み客データを製品のマルチテナントスキーマへ持ち込むことになり、テナント分離の前提が濁る | **人間が所在を決定**。Baseline で確定するのは原則のみ ——「**アウトリーチ用の見込み客データを、既存の顧客向けマルチテナントスキーマへ混在させない**」。所在そのものは決めず、未決事項として記録する |
...
- [A-13] CRM / opt-out ストアの所在（外部 SaaS か自社実装か）。外部なら越境移転と処理者契約、自社なら自社見込み客データの置き場所の設計が必要。Baseline で確定した原則は「アウトリーチ用の見込み客データを、既存の顧客向けマルチテナントスキーマへ混在させない」のみで、所在は未決。
...
本 Baseline PR では**修復しない**既知の問題を記録する。

### KI-01 `phase-N-complete` タグが存在しない

| 項目 | 内容 |
|---|---|
| **事象** | `phase-N-complete` タグが local clone・remote とも **0 本**。`CLAUDE.md` のリリースタグ方針（完了・マージ済みの各フェーズに annotated タグを付与する）および `docs/decisions/TODO.md` の記述（「local clone にのみ存在する」）と実態が食い違う。 |
| **影響** | 各フェーズの完了コミットを tag から特定できない。フェーズ完了報告で参照すべき tag 名・tag object SHA・target commit SHA が検証できず、過去フェーズの範囲をコミット履歴から再構成する必要がある。 |
| **対応方針** | **本 PR では修復しない。** 正しい historical target commit の検証が必要であり、**誤ったコミットへタグを付けると恒久的な誤りになる**（タグは移動・上書き・再利用が禁止されているため、事後の訂正が効かない）。別タスクとして起票し、人間承認のうえ実施する。 |
| **制約** | 本環境では tag push が **HTTP 403** で失敗する。したがって修復タスクを実施する場合も、ローカルの annotated tag 作成までを行い、remote への反映は手動コマンドを添えて人間へ引き渡す運用になる。 |
| **本 PR での扱い** | タグの**作成も push も行わない**。`AGENT_GOVERNANCE.md` §5.1 にタグ push が 403 である旨を記述したのみで、これは記述であってタグ操作ではない。 |
