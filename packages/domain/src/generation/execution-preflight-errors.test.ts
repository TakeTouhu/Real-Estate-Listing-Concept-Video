...
describe("preflightFailureStateFor", () => {
  ...
  it("covers the whole reason vocabulary and nothing else", () => {
    ...
    // vocabulary: a sixteenth reason would make this fail rather than quietly
    // going untested.
  ...
  it("agrees with the disposition it derives from", () => {
    ...
    // Asserted as a property over all fifteen rather than as two hand-picked
  ...
  it("partitions the vocabulary — neither state is unreachable", () => {
    // A mapping that sent all fifteen to one state would pass every assertion
    ...
    // ASSET_SOURCE_UNIDENTIFIABLE joined the terminal side in Phase 4C-3A-2a,
    // taking it to ten; MODEL_UNAVAILABLE joined the retryable side in Phase
    // 4C-3B-2B, taking that to five.
