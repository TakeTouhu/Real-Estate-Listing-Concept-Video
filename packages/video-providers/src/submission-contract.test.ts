...
describe("the submission port is a narrow sub-port of the provider seam", () => {
  ...
  it("requires only submission and error normalization of a submission-only adapter", () => {
    // A submission-only adapter must be able to implement this and nothing
    // else. If the sub-port grew polling or pricing, such an adapter would have
    // to fabricate answers it does not have — the fabrication ADR-0033 refused
    // for unverified catalog entries.
    const keys: Assert<
